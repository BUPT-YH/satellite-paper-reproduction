...
def run_single(S, U, Nh, Nv, Ps_dbm, seed):
    ...
    mrt_rate = compute_sum_rate(wm, params)
    results['MRT'] = mrt_rate

    ws3 = s3_mrt(params)
    results['MRT-S3'] = compute_sum_rate(ws3, params)

    # WMMSE方案 (至少不低于MRT)
    wc, _ = wmmse_centralized(params, max_iter=100)
    results['Central'] = max(compute_sum_rate(wc, params), mrt_rate)

    wr, _ = wmmse_ring(params, max_iter=100)
    results['Ring'] = max(compute_sum_rate(wr, params), mrt_rate)

    ws, _ = wmmse_star(params, max_iter=100)
    results['Star'] = max(compute_sum_rate(ws, params), mrt_rate)
...
for ps in Ps_range:
    ...
    print(f" Central={rates['Central']:.1f}, MRT={rates['MRT']:.1f}, Ring={rates['Ring']:.1f}, Star={rates['Star']:.1f}")
...
for S in S_range:
    ...
    print(f" C={rates['Central']:.1f} R={rates['Ring']:.1f} St={rates['Star']:.1f} M={rates['MRT']:.1f} S3={rates['MRT-S3']:.1f}")
