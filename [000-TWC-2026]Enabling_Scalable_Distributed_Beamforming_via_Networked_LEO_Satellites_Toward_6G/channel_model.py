...
def _closed_form_update_sat(s, w, mu, nu, params):
    S, U, Nrf = params['S'], params['U'], params['Nrf']
    ab = params['alpha_bar']
    bv = params['beta_var']
    gm = params['gamma']
    ...
    for u in served:
        A = reg * np.eye(Nrf, dtype=complex)

        # Own-beam: LOS power + NLOS self-interference
        c_self = nu[u] * np.abs(mu[u]) ** 2 * (ab[s, u] ** 2 + bv[s, u])
        gc = g[s, u].conj()
        A += c_self * np.outer(gc, g[s, u])

        # Interference w[s,u] causes to other served users v
        for v in served:
            if v == u:
                continue
            c_v = nu[v] * np.abs(mu[v]) ** 2 * gm[s, v]
            A += c_v * np.outer(g[s, v].conj(), g[s, v])

        # Signal from other satellites (cross-satellite coherent combining)
        Fu_ms = 0.0
        for ss in range(S):
            if ss == s:
                continue
            Fu_ms += ab[ss, u] * np.dot(g[ss, u], delta[ss, u] * w[ss, u])

        # RHS: nu * mu* * ab * (1 - mu * Fu^{-s}) * g*
        q = 1.0 - mu[u] * Fu_ms
        b = nu[u] * mu[u].conj() * ab[s, u] * q * g[s, u].conj()

        try:
            w[s, u] = np.linalg.solve(A, b)
            ...
            for s in range(S):
                ...
                for u in served:
                    ...
                    for v in range(U):
                        if v == u:
                            continue
                        wu_for_v = delta[s, u] * w[s, u]
                        gwu_v = np.dot(g[s, v], wu_for_v)
                        grad += nu[v] * np.abs(mu[v]) ** 2 * 2 * gm[s, v] * gwu_v.conj() * g[s, v]
                    w_local[u] = w[s, u] - 0.005 * grad
